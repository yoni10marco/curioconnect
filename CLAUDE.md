...
- `get-leaderboard` — fetches top 100 users from `leaderboard_profiles` view sorted by XP or `effective_streak`; the view computes streak validity at read time via `get_effective_streak()` so stale streaks (user hasn't opened app) show as 0; XP tab filters `total_xp > 0`, streak tab filters `effective_streak > 0` (users with 0 in both are excluded); current user is always appended to the result if not already in the list
